      ...
      <div>
        ...
        >
          ...
          >
            <p
              :class="{
                green: row.direction === 'Buy',
                red: row.direction === 'Sell',
              }"
            >
              {{ row.direction }}
            </p>
          ...
          >
            {{ row }}
            <!-- <p :class="[plusAndMinus(row.profit).className]">
                {{ plusAndMinus(row.profit).sign }}${{ row.profit | subRadio }}
              </p> -->
          </ElTableColumn>
          <ElTableColumn
            prop="margin"
          ...
          >
            {{ row }}
            <!-- {{ row.open_time | timestampDate }} -->
          </ElTableColumn>
          <!-- <ElTableColumn prop="address" label="Action" min-width="120">
            <template #default="{ row, index }">
              <ElButton
                round
                class="sty5-btn grey sty6-btn taplight"
                :loading="row.loading"
